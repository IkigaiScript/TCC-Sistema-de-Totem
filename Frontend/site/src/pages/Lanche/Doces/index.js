...
export default function Doce (){
    ...
    async function consultBebida(){
        try{
            const response = await snackbar.consultProduto('doce');
            setDoces([...response]);
            return response;
        }
        catch(e){
    ...
    function switchRoutes(route){
        if(route !== 'master'){
            window.location.replace(`${window.location.origin}/compra/lanche/${route}`);
        }
    }
        ...
        <PagesDefault>
            ...
            <ButtonWrapper>
                ...
                <Select onClick={e => switchRoutes(e.target.value)}>
                    ...
                    <Option selected value='master'> Doce </Option>
